...
while hrdina.je_nazivu() or vyhral_jsi == "ano":
    ...
    if volba ==3:
        pokrok += 1
        #if pokrok < 4:
            #seznam_ukolu=[hlavni_dejova_linka.ukol1.hadej,hlavni_dejova_linka.ukol2.hadej_hadanku,hlavni_dejova_linka.ukol3.kamen_nuzky_papir]
            #vybrany_ukol=random.choice(seznam_ukolu)
            #vybrany_ukol()
            #seznam_ukolu.remove(vybrany_ukol)
        if pokrok == 1:
        ...
        if pokrok == 5:
            ...
            if hrdina.je_nazivu():
                print("Gratuluji, vyhrál jsi a princezna je tvoje ")
                break
