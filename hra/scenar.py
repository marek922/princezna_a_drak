...
class Princezna_a_drak:
    def hraj(self):
        ...
        while hrat == "ano":
            ...
            conn = sqlite3.connect('nejlepsi_skore.db') #připojení k databázi
            cur = conn.cursor()
            cur.execute('''CREATE TABLE IF NOT EXISTS vyherci (
                        "cislo_hry" INTEGER PRIMARY KEY AUTOINCREMENT,
                        "jmeno" TEXT,
                        "zivoty" INTEGER,
                        "utok" INTEGER,
                        "obrana" INTEGER,
            ...
            while celkem != pokus_cislo and pokus_cislo != 0:  # postupné přiřazování hodnot pomocí funkce "zadání hodnot" k atribitům a následná kontrola, za byl přiřazen správný počet
                zivot_dodatek = hlavni_dejova_linka.zadani_hodnot(
                    ("Kolik bodů přičteš k životům? Čím více, tím odolnější budeš\n"))
                utok_dodatek = hlavni_dejova_linka.zadani_hodnot(
                    "Kolik bodů přičteš k útoku? Čím více, tím větší rány budeš rozdávat\n")
                obrana_dodatek = hlavni_dejova_linka.zadani_hodnot(
                    "Kolik bodů přičteš k obraně? Čím více, tím silnější tvůj štít bude\n")
                inteligence_dodatek = hlavni_dejova_linka.zadani_hodnot(
                    "Kolik bodů přičteš k inteligenci? Důvtip by se neměl podceňovat\n")
                jidlo_dodatek = hlavni_dejova_linka.zadani_hodnot(
                    "Kolik bodů přičteš k zásobě jídla? Čím více, tím více ho budeš mít a jídlo se vždy hodí:)\n")
                print()
                ...
                if celkem > pokus_cislo:
                    pokus_cislo -= 1
                    if pokus_cislo > 0:
                        print(f"""Kdepak, podvádět se nevyplácí. Říkal jsem ti, jen {pokus_cislo+1}. Tobě to bylo málo co?
            ...
            while hrdina.je_nazivu():  # samotný průběh hrou
                ...
                if volba == 1:
                    ...
                    if delka_chuze == 1:  # spustí procházení v lese s nejniží obtížností
                        ...
                        print(
                            " Jsi rebel a zvolil jsi možnost, která nebyla na výběr , takže se vůbec nic nestalo. Příště zvol možnost 1 / 3 / 5")
                ...
                if volba == 3:  # spuštění samotných úkolů důležitých o záchraně princezny
                    ...
                    if pokrok == 5:
                        souper=drak
                        ukol5 = hlavni_dejova_linka.Ukol_5(hrdina,souper)
                        print(ukol5)
                        print()
                        print(f" Utočíš na {souper.jmeno} s {souper.zivoty} životy. {souper.jmeno} má sílu {souper.utok} a brání se za {souper.obrana} bodů")
                        print()
                        ukol5.boj_s_drakem()
                        ...
                        if hrdina.je_nazivu():
                            ...
                            cur.execute("INSERT INTO vyherci (jmeno, zivoty, utok, obrana, obtiznost, vysledek, minuty, vteriny) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                                (hrdina.jmeno, hrdina.zivoty, hrdina.utok, hrdina.obrana, obtiznost, "Vyhra",
                                 cas_dohrani_minuty, cas_dohrani_vteriny))# zapsání infomací o výsledkách hry do databáze
                            conn.commit()
                            conn.close()
                            break
            ...
            if hrdina.je_nazivu():
                ...
                cur.execute("INSERT INTO vyherci (jmeno, zivoty, utok, obrana, obtiznost, vysledek, minuty, vteriny) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",(hrdina.jmeno, hrdina.zivoty, hrdina.utok, hrdina.obrana, obtiznost, "Prohra", cas_dohrani_minuty,cas_dohrani_vteriny))
                #zapsání vsýledku hry do databáze
                conn.commit()
                conn.close()
            print()
            hrdina.restart_postavy()

            prace_s_databazi.databeze.nejlepsich_deset_podle_urovne(obtiznost)
